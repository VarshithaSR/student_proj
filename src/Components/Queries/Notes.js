  ...
  return (
    <main>
      ...
      <ul className="grid grid-cols-1 font-semibold sm:grid-cols-2 lg:flex lg:items-center lg:justify-start lg:gap-16">
        ...
        <li>
          <Link
            className="rounded-md px-2 py-1 underline decoration-orange-900  decoration-2 underline-offset-2 hover:bg-orange-950 hover:text-slate-100 hover:decoration-0 dark:decoration-inherit dark:hover:bg-orange-900/80 dark:hover:text-slate-200 md:p-2 "
            to="students"
          >
            Students
          </Link>
      ...
      <section className="note__body w-full ">
        {notes?.map((note, index) => (
          <article
            className="mt-4 overflow-auto whitespace-break-spaces rounded-md border-2 border-slate-900 bg-orange-200 dark:border-slate-500 dark:bg-slate-950 dark:text-slate-300"
            key={index}
          >
            <details>
                  ...
                  {user.userType === "teacher" && (
                    <div className="flex p-3 pb-1">
                      <Link to={`${index}/edit`} id={index}>
                        <FaEdit className="ml-2 rounded-md p-1 text-3xl hover:bg-orange-900 hover:text-slate-100 dark:hover:bg-orange-600 lg:p-2 lg:text-4xl" />
                      </Link>
                      <Link
                        id={note._id}
